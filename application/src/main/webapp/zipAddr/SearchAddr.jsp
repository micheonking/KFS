...
function init(){
	//var cls = new window.parent.myApp.client.test.GetJS('developpez.com');
	//alert(GetJs.getName());
	...
	if(inputYn != "Y"){
		...
		--%>
		opener.jusoCallBack("<%=roadAddrPart1%>", "<%=roadAddrPart2%>", "<%=addrDetail%>", "<%=zipNo%>");
		<%--
		--%>
		<%--
		var fn = window.parent.myApp.client.test.JSCaller.action3();
		alert(fn("<%=roadAddrPart1%>")); // will return 42!
		--%>

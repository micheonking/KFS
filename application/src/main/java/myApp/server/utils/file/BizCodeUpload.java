...
public class BizCodeUpload implements javax.servlet.Servlet {
	...
	private void uploadFile(HttpServletRequest request, HttpServletResponse response) throws FileUploadException, SQLException {
		...
		try {
			...
			while ((strLine = bufferedReader.readLine()) != null) {
				...
				Fnd03_ConsignmentModel consignmentModel = new Fnd03_ConsignmentModel();
				...
				for(int j=0; j<data.length; j++) {
					...
					switch(j) {
					case 0:		//수탁사코드
						consignmentModel = sqlSession.selectOne("fnd03_consignment.selectByCd", data[j]);
						
						if (consignmentModel == null) {
							consignmentModel = new Fnd03_ConsignmentModel();
							consignmentModel.setKeyId(isNewData.getSeq(sqlSession));
							consignmentModel.setCompanyId(companyId);
							consignmentModel.setConsignmentUseYn(false);
						}
						consignmentModel.setConsignmentCode(data[j]);
						break;
						
					case 1:	// 수탁사명
						consignmentModel.setConsignmentName(data[j]);
						break;
					}
				}
				sqlSession.insert("fnd03_consignment.insertUpdate",consignmentModel);
			}
		}

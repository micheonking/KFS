...
public class Student {
	...
	public void selectById(SqlSession sqlSession, ServiceRequest request, ServiceResult result){
		...
		List<GridDataModel> list = sqlSession.selectList(mapperName + ".selectById", studentId);
...
//		List<AbstractDataModel> updateList = new ArrayList<AbstractDataModel>(); 
		...
		for(GridDataModel data : request.getList()){
			...
			List<GridDataModel> updateOne = new ArrayList<GridDataModel>();  
			updateOne.add(data); 
			...
			if(classStudentModel.getClassStudentId() == null) {
				...
				List<GridDataModel> classStudentList = new ArrayList<GridDataModel>();

...
public class Apr01_Tab_MyApproveList extends BorderLayoutContainer {
	...
	private LabelToolItem label01 = new LabelToolItem("▶결재내역 조회");
	...
	public Apr01_Tab_MyApproveList() {
	...
	public Grid<Apr01_ApprModel> buildGrid(){
		...
		gridBuilder.addText(properties.title(), 320, "문서명");
...
//				dcrModel.setDocId(grid.getSelectionModel().getSelectedItem().getApprId());//문서분류 초기값 설정
//				dcrModel.setRegEmpId(LoginUser.getUserId()); // 등록자ID 설정
//				dcrModel.setRegEmpName(LoginUser.getUserName());//등록자명 설정
////				dcrModel.setActionCell("");
//				
//				Apr01_Edit_AprInsert editor = new Apr01_Edit_AprInsert(null, dcrModel, "edit");
//				editor.show();
			}
		});
		...
		gridBuilder.addDate(properties.myApprovalDate(), 90, "나의결재일");
	...
	private void setInitData() {
		...
		this.statusCode.setEditable(false);
	}
	...
	private void retrieve(){
		...
		service.addParam("status", statusCode.getCode());
		service.addParam("aprYn", aprCheckBox.getValue());
		service.retrieve("apr.Apr01_Appr.selectApprListByEmpId", new InterfaceCallback() {
			@Override
			public void execute() {
			}
		});

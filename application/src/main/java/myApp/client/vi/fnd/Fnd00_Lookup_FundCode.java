...
public class Fnd00_Lookup_FundCode extends Window implements InterfaceServiceCall {
	...
	public void open (String date, InterfaceCallbackResult callback) {
		workDate = ClientDateUtil.toDate(date);;
		open(callback);
	}
	...
	public void open (InterfaceCallbackResult callback) {
		...
		FieldLabel searchField = new FieldLabel(fundNameText, "검색 ");
...
//		gridBuilder.addText(properties.planYn(), 120, "상품구분");
	...
	private void retrieve() {
		...
		service.addParam("workDate", workDate);

		service.retrieve("fnd.Fnd00_Fund.selectByLookupFundCode", new InterfaceCallback() {
			@Override
			public void execute() {
				grid.unmask();
			}
